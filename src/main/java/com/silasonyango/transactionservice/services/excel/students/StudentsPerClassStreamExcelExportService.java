...
import static com.silasonyango.transactionservice.common.config.UtilityConfigs.STUDENTS_PER_CLASS_STREAM_EXCEL_SHEET_NAME;

@Service
public class StudentsPerClassStreamExcelExportService {
    ...
    public XSSFWorkbook processData(XSSFWorkbook workbook, String sheetTitle, List<Map<String, Object>> dataList) {
        XSSFSheet sheet = workbook.getSheet(STUDENTS_PER_CLASS_STREAM_EXCEL_SHEET_NAME);
        workbook = writeHeaderLine(sheet.getLastRowNum() + 2, sheetTitle, workbook, STUDENTS_PER_CLASS_STREAM_EXCEL_SHEET_NAME);
        workbook = writeDataLines(dataList, sheet.getLastRowNum() + 1, workbook, STUDENTS_PER_CLASS_STREAM_EXCEL_SHEET_NAME);
        return workbook;
    }
}
